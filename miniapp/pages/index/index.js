Page({
  ...
  run: function() {
    var t = this;
    t.drawYearly(function() {
      t.drawMonthly(function() {
        t.drawDay(function() {
          t.drawType(function() {
            t.drawProvince(function() {
              t.drawKeyword(function() {});
            });
          });
        });
      });
  ...
  // 画网格
  makeGrid: function(c, w, h, pl, pt, pb, pr, mv) {
    ...
    for (var i = 0; i <= 4; i++) {
      ...
      c.fillText(String(Math.round(mv * i / 4)), pl - 6, y + 4);
    }
  },

  drawYearly: function(cb) {
    ...
    this.makeGrid(c, w, h, pl, pt, pb, pr, mv);
    ...
    for (var i = 0; i < 6; i++) {
      ...
      c.fillText(String(v), x + bw / 2, y - 8);
      c.setFillStyle('#666');
      c.setFontSize(12);
  ...
  drawDay: function(cb) {
    ...
    for (var i = 0; i < n; i++) {
      ...
      if (i % 3 === 0) {
        ...
        c.setFontSize(9);
        c.setTextAlign('center');
        c.fillText(String(i + 1), x + bw / 2, h - 6);
      }
    }
    c.draw(false, function() {
      if (cb) setTimeout(cb, 50);
  ...
  drawType: function(cb) {
    ...
    c.setFontSize(18);
    c.setTextAlign('center');
    c.fillText(String(total), cx, cy + 5);
    var lx = w * 0.52, ly = 15;
    for (var i = 0; i < 7; i++) {
      c.setFillStyle(cl[i]);
      ...
      c.setFontSize(12);
      c.setTextAlign('left');
      var p = Number((d[i].v / total) * 100).toFixed(1);
  ...
  drawProvince: function(cb) {
    ...
    for (var i = 0; i < n; i++) {
      ...
      c.setFontSize(12);
      c.setTextAlign('left');
      c.fillText(String(d[i].v), pl + bw + 8, y + bh / 2 + 4);
    }
    c.draw(false, function() {
      if (cb) setTimeout(cb, 50);
    });
